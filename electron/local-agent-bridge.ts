...
export class LocalAgentBridge extends EventEmitter {
  ...
  async startAgent(config: LocalAgentConfig): Promise<string> {
    ...
    const workerPath = path.join(process.cwd(), 'src/modules/collaboration-runtime/sandbox/agent-worker.mts');
    // process.execPath in Electron is the Electron binary, not Node.js.
    // Use the tsx CLI directly so the worker TypeScript file can be executed by a real Node process.
  ...
  private handleWorkerOutput(agentId: string, chunk: string): void {
    ...
    for (const line of lines) {
      ...
      try {
        ...
        // non-JSON line from worker (e.g. diagnostic text) — ignore
      }
    }
  }
