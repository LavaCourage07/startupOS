...
当前 `SessionStore` 的源码只能直接证明会话身份与持久化关系，不能证明实际 Web 窗口如何绑定 session。教材中的窗口关联图是用于解释对象边界的模型；具体绑定实现必须到窗口管理模块和组件调用点另行验证。这种“能从源码确认到什么”与“架构上合理推断什么”的区分，能避免把设计图误写成已实现调用链。
