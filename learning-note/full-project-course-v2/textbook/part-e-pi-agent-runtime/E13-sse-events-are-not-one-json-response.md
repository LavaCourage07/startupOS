...
当前源码树中没有发现直接导入 `parseSSE`、并用“半帧、多个完整帧、尾部残帧、无效 JSON、多字节中文跨 chunk”逐项断言的单元测试。`parseSSE` 还是 `client-hooks.ts` 内部函数，这使它更难被独立验证。现有 Hook 会话隔离测试主要用 mock 事件驱动状态，不等于真实地把字节流喂给 SSE parser。

因此，本节可以从源码确认 buffer 与事件分隔算法，却不能声称网络分片边界已经被自动化测试完整覆盖。更直接的测试应把读取与解析抽成可控函数，至少验证：

| 输入 | 期望 |
| --- | --- |
| 一个事件被拆成两个 chunk | 第一块不解析，第二块到来后只产出一个事件 |
| 一个 chunk 含两个完整事件和半个事件 | 先产出两个，半个留在 buffer |
| 中文字符的 UTF-8 字节跨 chunk | `TextDecoder({ stream: true })` 后字符不损坏 |
| `data:` 中是非法 JSON | 错误有明确策略，不把后续合法帧一起丢失 |
| 网络结束但没有协议 `done` | 标记为异常收尾，而不是业务成功 |

这是“源码逻辑看起来合理”和“边界已被测试固定”之间的差别。

## 12. 纸面实验

把下面两次网络读取写在纸上：第一次得到 `data: {"type":"text_`，第二次得到 `delta","data":{"delta":"西湖"}}\n\n`。第一次读取后不能调用 JSON.parse；第二次拼接后才能得到一个完整事件。再追加半个 `done` 帧，说明它为什么必须继续留在 buffer。

若读者能同时指出字节解码边界、SSE 帧边界和 JSON 解析边界，才算真正理解这段代码，而不是只记住 `getReader()`。
