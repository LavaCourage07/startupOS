...
OriginOS 把业务理解、解决方案设计和 AI 执行连接成一个连续闭环：

### 1. 项目访谈：理解业务

从业务目标出发，而不是从空白工作流开始。Project Agent 通过访谈和资料读取，澄清角色、实体、规则、行动与约束，并把这些上下文沉淀为可见的业务模型和本体。

**阶段产出：** 包含共享上下文、来源文件、业务事实和本体模型的项目工作空间，后续 Agent 可以基于这些事实工作。

### 2. AI 解决方案：设计业务如何被执行

基于业务模型设计可执行的 AI 解决方案。明确参与的 Agent、RoleAgent 和技能，定义任务如何拆分、各自承担什么职责、需要遵守哪些契约与检查点，以及哪些结果必须经过人工审核。

**阶段产出：** 将业务目标连接到任务、职责、工具、验收标准和预期产物的解决方案。

### 3. AI 运行业务：执行、审核与知识沉淀

在 Agent 或多 Agent 运行时中执行解决方案。Agent 根据项目事实和约束推进任务，调用技能与工具，持续展示进度和审核节点，并将结果写回工作空间。完成的工作还可以沉淀为可复用技能、知识和业务模式。

**阶段产出：** 可追溯的执行过程、可审核的决策、业务产物，以及可以持续复用的组织知识。

> **项目访谈 → 业务模型与本体 → AI 解决方案 → Agent 执行 → 业务产物与知识**
