...
async function notifyReleaseService(version, baseUrl, prefix) {
  ...
  if (!releaseApiUrl) {
    console.log('[publish-all-v0.1.12] ORIGINOS_RELEASE_API_URL not set, skipping release service notification');
    return;
  }

  if (!releaseApiKey) {
    console.warn('[publish-all-v0.1.12] ORIGINOS_RELEASE_API_KEY not set, skipping release service notification');
    return;
  }
  ...
  console.log('[publish-all-v0.1.12] notifying release service', {
    url: releaseApiUrl,
  ...
  try {
    ...
    console.log('[publish-all-v0.1.12] release service notified successfully', result);
  } catch (error) {
    console.error('[publish-all-v0.1.12] failed to notify release service:', error instanceof Error ? error.message : error);
  }
}
...
async function main() {
  ...
  console.log('[publish-all-v0.1.12] release', {
    version,
    bucket,
    prefix,
    region,
  ...
  for (const artifact of artifacts) {
    ...
    if (!artifact.overwrite) {
      ...
      if (stat.exists) {
        console.log(`[publish-all-v0.1.12] skipping existing ${artifact.fileName}`);
        continue;
      }
    }

    console.log(`[publish-all-v0.1.12] uploading ${artifact.fileName} -> ${key}`);
    ...
    if (url && !skipCdnVerify) {
      console.log(`[publish-all-v0.1.12] verifying ${url}`);
  ...
  console.log('[publish-all-v0.1.12] published successfully');
}

main().catch((error) => {
  console.error('[publish-all-v0.1.12] failed:', error instanceof Error ? error.message : error);
  process.exitCode = 1;
});
