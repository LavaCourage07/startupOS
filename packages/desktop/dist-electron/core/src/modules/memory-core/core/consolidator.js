...
const history_store_1 = require("../recall/history-store");
const archival_memory_1 = require("../archival/archival-memory");
const pattern_ingest_1 = require("../archival/pattern-ingest");
class MemoryConsolidator {
    constructor(agentDir, sessionId = 'default', modelFactory) {
        this.memory = new memory_1.Memory(agentDir);
        this.history = new history_store_1.HistoryStore(node_path_1.default.join(agentDir, 'memory', 'history'), sessionId);
    ...
    async analyzeRecentHistory(turns) {
...
Respond in the same language as the conversation (Chinese if conversation is in Chinese).`;
        try {
            const factory = this.modelFactory ?? (await Promise.resolve().then(() => __importStar(require('../../../lib/integrations/pi-agent/server-config'))));
    ...
    async ingestReflections(turns) {
        ...
        for (const turn of turns) {
            ...
            await (0, pattern_ingest_1.ingestReflectionToArchival)(this.archival, {
    ...
    extractKnowledgeCandidates(turns) {
        return turns
            .map((turn) => {
            const entities = [];
            const facts = [];
