...
class DependencyChecker {
     ...
     */
    deriveDependenciesFromTopology(agentId, topology) {
        const dependencies = [];
     ...
     * 推导指定任务的依赖（基于 DAG 层级）
     */
    deriveDependenciesFromDag(agentId, allTasks) {
        const dependencies = [];
     ...
     * 过滤已满足的依赖
     */
    filterSatisfiedDependencies(dependencies) {
        const satisfied = [];
        const missing = [];
        for (const dep of dependencies) {
     ...
     */
    isTransitivelyBlocked(dependencies) {
        for (const dep of dependencies) {
            // 检查上游 Agent 是否阻塞
            const blockedKey = (0, memory_keys_1.buildWorkerKey)(memory_keys_1.MemoryKeyCategory.BLOCKED, dep.agentId);
     ...
     */
    generateResolutionEvent(taskId, workerId, resolvedDeps) {
        return {
     ...
     * 获取阻塞原因的详细描述
     */
    getBlockedReasonDetails(workerId) {
        const blockedKey = (0, memory_keys_1.buildWorkerKey)(memory_keys_1.MemoryKeyCategory.BLOCKED, workerId);
        const blockedEntry = this.blackboard.getDataEntry(blockedKey);
        if (!blockedEntry?.value) {
            return null;
        }
        const blocked = blockedEntry.value;
    ...
    checkSingleDependency(dep) {
        if (dep.type === "task-complete" && dep.taskId) {
            ...
            // 检查 Agent 是否有任何已完成任务
            const tasks = this.blackboard.getTasks();
            const agentCompletedTasks = tasks.filter((t) => t.assignedTo === dep.agentId && (t.status === "completed" || t.status === "reported"));
     ...
     * 格式化阻塞原因
     */
    formatBlockedReason(missingDeps) {
     ...
     * 检查 Agent 可用的上游输出
     */
    getAvailableUpstreamOutputs(agentId) {
