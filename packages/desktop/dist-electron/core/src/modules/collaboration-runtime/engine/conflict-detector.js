...
class ConflictDetector {
    constructor(blackboard, _eventStore, config) {
        this.history = [];
        this.recentWrites = new Map(); // key → last writer
     ...
     */
    detectGoalConflict(event) {
        // MVP: 检测同一任务同时有成功和失败事件
        const taskId = event.payload?.["taskId"];
        if (!taskId)
            return null;
        const relatedEvents = this.history.filter((c) => c.details["taskId"] === taskId && c.type === "goal_conflict");
     ...
     */
    resolveDataConflict(conflict) {
        const key = conflict.details["key"];
        if (!key) {
     ...
     */
    resolveDeadlock(conflict) {
        const cycle = conflict.details["cycle"];
    ...
    selectDeadlockVictim(agents) {
        ...
        const priorityMap = new Map(this.config.agentPriorities.map((p) => [p.agentId, p.priority]));
        let victim = agents[0];
