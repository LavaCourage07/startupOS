...
exports.SupervisorMode = exports.CapabilityMatcher = void 0;
class CapabilityMatcher {
    ...
    scoreAgent(agent, task) {
        ...
        if (taskDesc !== undefined && taskDesc.length > 0 && agentDomain !== undefined && agentDomain.length > 0) {
            ...
            const domainMatch = domainWords.some((w) => descWords.some((dw) => dw.includes(w) || w.includes(dw)));
            if (domainMatch) {
                score += 20;
            }
        }
...
class SupervisorMode {
    ...
    async start(goal) {
        ...
        if (this.plan === null) {
            throw new Error("Plan unexpectedly null after start");
        }
        return this.plan;
    }
    /**
     * 标记 Worker 完成子任务。
     */
    markSubTaskComplete(taskId, result) {
     ...
     * 标记子任务执行失败。
     */
    markSubTaskFailed(taskId, error) {
        const task = this.plan?.subTasks.find((t) => t.id === taskId);
        if (task === undefined) {
            return;
        }
     ...
     */
    handleVerificationFailure(task, _check) {
    ...
    aggregate() {
        ...
        const completed = this.plan.subTasks.filter((t) => t.state === "completed").length;
        const failed = this.plan.subTasks.filter((t) => t.state === "failed").length;
        const escalated = this.plan.subTasks.filter((t) => t.state === "reported").length;
        this.plan.state =
    ...
    async allocateAll() {
        ...
        const pendingTasks = this.plan.subTasks.filter((t) => t.state === "pending");
    ...
    async allocateTask(task) {
        ...
        const candidates = this.capabilityMatcher.match(taskDesc, this.deps.agents.filter((a) => a.agentId !== "supervisor"));
        ...
        const verifierCandidates = this.deps.agents.filter((a) => a.agentId !== selectedWorker.agentId);
        ...
        try {
            const convId = await this.contractNet.callForProposal(taskDesc, [selectedWorker.agentId], taskDesc.deadline, this.deps.blackboard);
            // 模拟 Worker 接受（实际由 Worker 自行投标）
            this.contractNet.acceptProposal(convId, selectedWorker.agentId, this.deps.blackboard);
        }
        catch {
...
exports.SupervisorMode = SupervisorMode;
