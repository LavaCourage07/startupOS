...
exports.wrapWorkerHumanReviewRequest = wrapWorkerHumanReviewRequest;
exports.executeMultiAgentDag = executeMultiAgentDag;
exports.loadProjectTopology = loadProjectTopology;
exports.computeTaskLevels = computeTaskLevels;
exports.verifierFallbackResult = verifierFallbackResult;
exports.resumeSupervisorHitl = resumeSupervisorHitl;
exports.executeSupervisorDag = executeSupervisorDag;
exports.executeCollaborationRuntime = executeCollaborationRuntime;
...
const mode_router_1 = require("../../../modules/collaboration-runtime/engine/mode-router");
...
const streamSimple = pi_ai_1.streamSimple;
function wrapWorkerHumanReviewRequest(event, sessionId) {
...
async function findLatestManifestDir(projectId) {
    // 兼容两种 projectId 格式：有 proj- 前缀和没有前缀
    const candidates = [
        path_1.default.join((0, paths_1.getDataRoot)(), `projects/${projectId}/solutions`),
        path_1.default.join((0, paths_1.getDataRoot)(), `projects/proj-${projectId}/solutions`),
    ];
    for (const solutionsDir of candidates) {
        try {
            const entries = await (await Promise.resolve().then(() => __importStar(require("fs/promises")))).readdir(solutionsDir, { withFileTypes: true });
            const versionDirs = entries
                .filter((e) => e.isDirectory() && e.name.match(/^v\d+\.\d+$/))
                .sort((a, b) => b.name.localeCompare(a.name, undefined, { numeric: true }));
            if (versionDirs.length > 0) {
                return path_1.default.join(solutionsDir, versionDirs[0].name);
            }
        }
        catch {
            // 目录不存在，尝试下一个
...
async function loadAgentsJson(manifestDir) {
    const agentsPath = path_1.default.join(manifestDir, "agents.json");
    const content = await (0, promises_1.readFile)(agentsPath, "utf-8");
    ...
    for (const agent of agents) {
        if (agent.collaborations) {
            for (const collab of agent.collaborations) {
                if (!agentIds.has(collab.targetAgentId)) {
                    continue;
                }
 ...
 */
function normalizeEdgesForTopologyView(edgesInput) {
    const result = [];
    ...
    for (const edge of edgesInput) {
        ...
        if (hasPath(edge.to, edge.from)) {
            console.warn(`[Topology] preserving loop edge ${edge.from} -> ${edge.to} as notify for viewer`);
            result.push({
                ...edge,
                type: "notify",
            });
...
function buildTopology(agents, edges) {
    const manifest = {
        agents: agents.map(toManifestAgent),
        collaboration: { edges },
    };
    return (0, collaboration_runtime_1.parseTopology)(manifest);
}
/** 写入统一 manifest 供向后兼容（optional, 不影响执行） */
async function writeUnifiedManifest(projectId, agents, edges) {
    const solutionsDir = path_1.default.join((0, paths_1.getDataRoot)(), `projects/${projectId}/solutions`);
    await (0, promises_1.mkdir)(solutionsDir, { recursive: true });
    const manifestPath = path_1.default.join(solutionsDir, "solution-v1.0-manifest.json");
    const manifest = {
        version: "1.0.0",
        agents: agents.map(toManifestAgent),
        collaboration: { edges },
    };
    await (0, promises_1.writeFile)(manifestPath, JSON.stringify(manifest, null, 2), "utf-8");
}
/**
 * 执行多 Agent 协作 DAG。
 *
 ...
 */
async function executeMultiAgentDag(config, eventStore, eventEmitter) {
    // 1. 加载 manifest
    ...
    const blackboardDir = path_1.default.join((0, paths_1.getDataRoot)(), "projects", config.projectId, "collaboration-sessions", config.sessionId ?? "default");
    // 创建 Event Store 和 Blackboard
    let blackboard = await collaboration_runtime_1.Blackboard.loadSnapshot(config.sessionId ?? config.projectId, blackboardDir);
    if (!blackboard) {
        blackboard = new collaboration_runtime_1.Blackboard(config.sessionId ?? config.projectId, blackboardDir);
    }
    // UpstreamResults 通过 Blackboard 存储（Event Sourcing + Provenance）
    const upstreamResults = new collaboration_runtime_1.UpstreamResults(blackboard);
    const workerModel = (0, llm_config_1.runtimeLLMConfigToWorkerModel)(config.llmConfig);
    // 等待中的 Agent 进程引用（供 resume 使用）
    const waitingProcs = new Map();
    // Story 9.35: Lightweight Supervisor 懒加载状态
    const lsEnabled = config.enableLightweightSupervisor !== false;
    let lightweightSupervisorProc = null;
    /** 懒加载 Lightweight Supervisor 子进程（首次 WORKER_BLOCK 时触发） */
    async function ensureLightweightSupervisor(blockEvent) {
        if (!lsEnabled)
            return null;
        if (lightweightSupervisorProc) {
            // 复用已有进程：向其路由新的 WORKER_BLOCK
            try {
                await lightweightSupervisorProc.prompt(`[WORKER_BLOCK] ${JSON.stringify(blockEvent.payload)}`);
            }
            catch {
        ...
        const supervisorWorkingDir = path_1.default.join((0, paths_1.getDataRoot)(), "agents", "supervisor-lite");
    ...
    function emitSupervisorAgentStart(supervisorId) {
        const evt = {
    ...
    function buildAgentPrompt(agentId, globalGoal) {
    ...
    // 从 Agent 事件中提取审查请求
    function findReviewRequest(agentEvents, agentId) {
        return agentEvents.find((e) => e.type === "HUMAN_REVIEW_REQUEST" && e.payload?.["agentId"] === agentId);
    }
    // 从 Agent 事件中提取输出
    function extractAgentOutput(agentEvents) {
    ...
    // 4. 创建 DagExecutor
    const executor = new collaboration_runtime_1.DagExecutor(eventStore, 
    ...
    async (agentId) => {
        ...
        if (existingProc) {
            ...
            try {
                ...
                await (0, sandbox_1.getGlobalSpawner)().destroy(agentId);
            }
        }
        const spawner = (0, sandbox_1.getGlobalSpawner)();
        const workingDirectory = path_1.default.join((0, paths_1.getDataRoot)(), "projects", config.projectId, "agents", agentId);
        ...
        try {
            ...
            if (reviewRequest) {
                ...
                return {
                    status: "waiting",
                    reviewRequest: {
                        question: reviewRequest.payload?.["question"] ?? "",
                        context: reviewRequest.payload?.["context"] ?? {},
                    },
                };
            }
            // 提取 Agent 输出
    ...
    try {
        ...
        if (lightweightSupervisorProc) {
            const supervisorId = `supervisor-lite-${config.sessionId ?? config.projectId}`;
            try {
                await (0, sandbox_1.getGlobalSpawner)().destroy(supervisorId);
            }
            catch {
                // non-fatal
            }
...
async function loadProjectTopology(projectId) {
    ...
    try {
        ...
        const topology = await Promise.race([
            topologyPromise,
            new Promise((_, reject) => setTimeout(() => reject(new Error(`Topology build timeout after ${timeoutMs}ms`)), timeoutMs)),
        ]);
        return topology;
    }
    catch (err) {
 ...
 */
function computeTaskLevels(subTasks, topology) {
    ...
    const result = Array.from({ length: maxLevel + 1 }, () => []);
...
async function verifyTaskCompletion(taskDescription, agentMessages, modelFactory) {
    ...
    for (const msg of agentMessages) {
        ...
        for (const block of blocks) {
            if (block.type === "toolCall") {
                ...
                if (args?.filePath !== null && args?.filePath !== undefined) {
                    artifacts.push(args.filePath);
                }
...
请判断任务是否完成，以 JSON 格式回复。`;
    try {
        const factory = modelFactory ?? (await Promise.resolve().then(() => __importStar(require("../../../lib/integrations/pi-agent/server-config"))));
...
function verifierFallbackResult(textParts, artifacts, toolCallCount) {
...
function resumeSupervisorHitl(sessionId, userReply, workerId) {
    ...
    if (workerChannels && workerChannels.size > 0) {
        ...
        channel.resume(userReply).catch((err) => {
 ...
 */
async function executeSupervisorDag(config, eventStore, eventEmitter) {
    ...
    const attachmentsAbsDir = path_1.default.join(blackboardDir, "attachments");
    let blackboard = await collaboration_runtime_1.Blackboard.loadSnapshot(config.sessionId ?? config.projectId, blackboardDir);
    if (!blackboard) {
        blackboard = new collaboration_runtime_1.Blackboard(config.sessionId ?? config.projectId, blackboardDir);
    }
    const bb = blackboard;
    const upstreamResults = new collaboration_runtime_1.UpstreamResults(bb);
    const workerModel = (0, llm_config_1.runtimeLLMConfigToWorkerModel)(config.llmConfig);
    ...
    const workerCompletionCallbacks = new Map();
    const notifyWorkerCompletion = (workerId) => {
    ...
    const supervisorWorkDir = path_1.default.join((0, paths_1.getDataRoot)(), "projects", config.projectId);
    await (0, promises_1.mkdir)(supervisorWorkDir, { recursive: true });
    // 写入 project-collaboration-context.json（initializeSupervisorAgent 通过 loadProjectCollaborationContext 读取）
    const collabContextPath = path_1.default.join(supervisorWorkDir, "project-collaboration-context.json");
    const collabContext = {
    ...
    await (0, promises_1.writeFile)(collabContextPath, JSON.stringify(collabContext, null, 2), "utf-8");
    // 同时写入 Agent.md（Supervisor 身份定义）
    const agentMdPath = path_1.default.join(supervisorWorkDir, "Agent.md");
    try {
        await (0, promises_1.readFile)(agentMdPath, "utf-8");
        // Agent.md 已存在，不覆盖
    }
    catch {
...
`;
        await (0, promises_1.writeFile)(agentMdPath, agentMd, "utf-8");
    }
    // 4. 启动 Supervisor 子进程
    ...
    // Supervisor 完成信号
    let supervisorResolveDone;
    let supervisorRejectDone;
    const supervisorDonePromise = new Promise((resolve, reject) => {
        supervisorResolveDone = resolve;
        supervisorRejectDone = reject;
    });
    // 事件处理器 — 拦截 SUPERVISOR_TOOL_CALL 事件
    const onSupervisorEvent = (event) => {
        ...
        if (event.type === "HUMAN_REVIEW_REQUEST" && event.source === supervisorAgentId) {
            ...
            if (config.sessionId) {
                hitlResumerRegistry.set(config.sessionId, (reply) => {
                    const sup = (0, sandbox_1.getGlobalSpawner)().get(supervisorAgentId);
                    if (sup) {
                        sup.resume(reply).catch((err) => {
        ...
        if (event.type === "SUPERVISOR_TOOL_CALL") {
            ...
            // 异步处理工具调用，不阻塞事件处理器
            void handleSupervisorToolCall(toolCallId, toolName, args);
        }
    };
    // 工具调用处理器（SUPA-02 核心胶水逻辑）
    async function handleSupervisorToolCall(toolCallId, toolName, args) {
        ...
        try {
            switch (toolName) {
                case "dispatch_worker": {
                    ...
                    const workerSkills = (agent.skills ?? []).filter((skill) => !forbiddenTools.includes(skill));
                    const workingDirectory = path_1.default.join((0, paths_1.getDataRoot)(), "projects", config.projectId, "agents", workerId);
                    await (0, promises_1.mkdir)(workingDirectory, { recursive: true });
                    // 写入 Worker 的协作上下文
                    const workerCtxPath = path_1.default.join(workingDirectory, "project-collaboration-context.json");
                    const workerCtx = {
                        // 包含业务项目 ID，用于区分 OriginOS 业务项目和本体中的"项目"概念
                        originosProjectId: config.projectId,
                        ontologyId: `ontology-${config.projectId}`,
                    ...
                    await (0, promises_1.writeFile)(workerCtxPath, JSON.stringify(workerCtx, null, 2), "utf-8");
                    const workerEvents = [];
                    const captureWorkerEvent = (ev) => {
                        if (ev.type === "HITL_ESCALATE" && ev.source === workerId) {
                            ...
                            if (config.sessionId && workerProc) {
                                ...
                                sessionChannels.set(workerId, {
                                    resume: (reply) => workerProc.resume(reply),
                                    question,
                                    onBehalfOfName,
                                });
                                ...
                                hitlResumerRegistry.set(config.sessionId, (reply) => {
                                    workerProc.resume(reply).catch((err) => {
                        ...
                        if (ev.type === "AGENT_END" && ev.source === workerId) {
                            ...
                            if (workerResult) {
                                ...
                                const endMessages = ev.payload?.["messages"];
                            ...
                            void (async () => {
                                const snap = await collaboration_runtime_1.Blackboard.loadSnapshot(config.sessionId ?? "supervisor", blackboardDir);
                                if (snap) {
                    ...
                    try {
                        ...
                        failedAgents.push(workerId);
                        resultJson = JSON.stringify({ error: `Failed to spawn worker: ${spawnErr.message}` });
                    }
                    break;
                }
                case "wait_workers": {
                    const workerIds = Array.isArray(args["workerIds"]) ? args["workerIds"] : [];
                    const timeoutMs = typeof args["timeoutMs"] === "number" ? args["timeoutMs"] : 300000;
                ...
                case "cancel_worker": {
                    ...
                    if (workerId) {
                        ...
                        if (wResult?.proc) {
                            try {
                                await spawner.destroy(workerId);
                            }
                            catch { /* ignore */ }
                            wResult.status = "failed";
                            notifyWorkerCompletion(workerId);
                ...
                case "resume_worker": {
                    ...
                    try {
                        ...
                        resultJson = JSON.stringify({ error: resumeErr.message });
                    }
                    break;
                }
                case "escalate_to_human": {
                    const question = String(args["question"] ?? "");
                    const mergedContext = args["mergedContext"];
                    ...
                    const hitlEvent = {
                        ...
                        payload: {
                            question,
                            agentId: "supervisor",
                            onBehalfOf: mergedContext?.["onBehalfOf"] ?? "supervisor",
    ...
    try {
        ...
        throw new Error(`Failed to spawn supervisor: ${spawnErr.message}`);
    }
    ...
    try {
        ...
        const attachFiles = readdirSync(attachmentsAbsDir).filter((f) => !f.startsWith(".") && f !== "README.md");
        if (attachFiles.length > 0) {
            const fileLines = attachFiles.map((f) => `  - ${path_1.default.join(attachmentsAbsDir, f)}`).join("\n");
...
function buildWorkerPrompt(agent, specificAction, acceptanceCriteria, upstreamOutputs, topology, allAgents, attachmentsAbsDir) {
    let prompt = `【具体任务】\n${specificAction}\n\n【完成判定】\n${acceptanceCriteria}`;
    const dependencies = topology.edges
    ...
    if (attachmentsAbsDir) {
        ...
        let fileList = "";
        try {
            const files = readdirSync(attachmentsAbsDir).filter((f) => !f.startsWith(".") && f !== "README.md");
            if (files.length > 0) {
                fileList = files.map((f) => `  - ${path_1.default.join(attachmentsAbsDir, f)}`).join("\n");
            }
        }
        catch {
            // 目录不存在或无法读取，跳过
...
function extractAgentOutputFromEvents(agentEvents) {
 ...
 */
async function executeCollaborationRuntime(config, eventStore, eventEmitter, executionMode) {
    const manifestDir = await findLatestManifestDir(config.projectId);
    if (manifestDir === null) {
        throw new Error(`No solution manifest found for project ${config.projectId}`);
    }
    const agents = await loadAgentsJson(manifestDir);
    const edges = extractEdges(agents);
    const mode = executionMode ?? (0, mode_router_1.selectExecutionMode)({
