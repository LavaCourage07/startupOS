...
exports.ContractNetProtocol = void 0;
...
class ContractNetProtocol {
     ...
     */
    async callForProposal(task, candidates, deadline, blackboard) {
     ...
     * 将投标记录添加到会话中。
     */
    propose(conversationId, agentId, bid, blackboard) {
     ...
     */
    acceptProposal(conversationId, winnerAgentId, blackboard) {
        const session = this.sessions.get(conversationId);
        if (!session) {
            throw new Error(`ContractNet session ${conversationId} not found`);
        }
        ...
        if (!winningBid) {
            throw new Error(`Agent ${winnerAgentId} did not propose for ${conversationId}`);
        }
     ...
     * Supervisor 拒绝所有投标。
     */
    rejectAll(conversationId, blackboard) {
        const session = this.sessions.get(conversationId);
        if (!session) {
            throw new Error(`ContractNet session ${conversationId} not found`);
        }
     ...
     * 选择最佳投标（最高置信度）。
     */
    selectBestBid(conversationId) {
        const session = this.sessions.get(conversationId);
        if (!session || session.bids.length === 0) {
            return undefined;
        }
        return session.bids.reduce((best, current) => current.confidence > best.confidence ? current : best);
    }
    /**
     * 标记会话完成。
     */
...
exports.ContractNetProtocol = ContractNetProtocol;
