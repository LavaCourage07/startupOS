...
class Blackboard {
    ...
    applyEvent(event) {
        switch (event.type) {
            ...
            case "TASK_ASSIGNED": {
                ...
                if (task) {
                    task.assignedTo = event.payload["agentId"];
    ...
    mapEventTypeToMessageType(eventType) {
        switch (eventType) {
            case "AGENT_REQUEST":
                return "request";
            case "AGENT_RESPONSE":
    ...
    setData(key, value, agentId, options) {
        if (this.isLocked(key) && this.locks[key]?.holder !== agentId) {
            throw new Error(`Blackboard key "${key}" is locked by ${this.locks[key]?.holder}`);
        }
        const existing = this.sharedData[key];
        const nextVersion = existing ? existing.provenance.version + 1 : 1;
     ...
     */
    correctData(key, newValue, correctorId, reason) {
        const existing = this.sharedData[key];
        if (!existing) {
     ...
     */
    makeEntry(value, agentId, timestamp, eventPayload, version) {
        const provenance = {
            writer: agentId,
            timestamp,
            sourceUri: eventPayload["sourceUri"] ?? undefined,
            toolCallsCited: eventPayload["toolCallsCited"] ?? undefined,
            version,
        };
        return {
            value,
    ...
    // Locks (§3.3)
    // ==========================================================================
    lock(key, agentId, ttlMs = DEFAULT_LOCK_TTL_MS) {
    ...
    getMessages(agentId) {
        return this.messages
            .filter((m) => m.to === agentId || m.to === "*" || m.from === agentId)
            .map((m) => ({
            id: m.id,
    ...
    mapPerformativeToMessageType(performative) {
        const map = {
            inform: "inform",
            request: "request",
            propose: "propose",
    ...
    mapMessageTypeToPerformative(type) {
        const map = {
            inform: "inform",
            request: "request",
            propose: "propose",
    ...
    // Tasks (§3.3)
    // ==========================================================================
    createTask(description, dependsOn) {
    ...
    failTask(taskId, reason) {
        ...
        if (task.status === "completed")
            throw new Error(`Task ${taskId} cannot be failed (status: ${task.status})`);
        task.status = "failed";
        task.completedAt = new Date().toISOString();
        if (reason !== undefined) {
    ...
    // Artifacts
    // ==========================================================================
    addArtifact(name, producer, data, options) {
    ...
    setArtifact(name, data, producer, options) {
    ...
    async snapshot() {
        ...
        await promises_1.default.writeFile(path_1.default.join(this.snapshotDir, "blackboard.json"), JSON.stringify(dataFile, null, 2), "utf-8");
    }
    static async loadSnapshot(sessionId, snapshotDir) {
        try {
            const filePath = path_1.default.join(snapshotDir, "blackboard.json");
