...
exports.agentSessionService = exports.AgentSessionService = void 0;
...
class AgentSessionService {
    ...
    async saveSession(session) {
        ...
        await this.store.write(this.getSessionPath(session.sessionId, projectId), session);
    }
    /**
     * Get session by ID
     */
     ...
     */
    async updateSession(sessionId, updates, projectId) {
     ...
     */
    async addMessage(sessionId, message, projectId) {
        const session = await this.getSession(sessionId, projectId);
        if (!session) {
            return null;
        }
     ...
     * Get session summary
     */
    async getSessionSummary(sessionId, projectId) {
        const session = await this.getSession(sessionId, projectId);
        if (!session) {
            return null;
        }
     ...
     */
    async autoGenerateSummary(sessionId, projectId) {
        const session = await this.getSession(sessionId, projectId);
        ...
        const firstUserMessage = userMessages[0]?.content?.substring(0, 50) || 'No content';
    ...
    generateSummary(session) {
        ...
        const toolCalls = session.messages.reduce((sum, m) => sum + (m.toolResults?.length ?? 0), 0);
        const firstMessage = session.messages[0]?.content?.substring(0, 100);
        const lastMessage = session.messages[session.messages.length - 1]?.content?.substring(0, 100);
        return {
            totalMessages,
            userMessages,
            assistantMessages,
     ...
     */
    getSessionPath(sessionId, projectId) {
        if (projectId) {
 ...
 */
exports.agentSessionService = new AgentSessionService();
