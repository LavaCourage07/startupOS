...
async function handle(context) {
    ...
    const instruction = input?.data || input?.message || '';
    if (!instruction) {
        return {
            success: false,
            error: {
 ...
 */
async function handleCreateTask(instruction, tools) {
 ...
 */
async function handleUpdateStatus(instruction, tools) {
 ...
 */
async function handleAssignTask(instruction, tools) {
    const taskId = extractTaskId(instruction);
    if (!taskId) {
        return {
            success: false,
 ...
 */
async function handleListTasks(instruction, tools) {
    let where = {};
    // Filter by status
    const status = parseTaskStatus(instruction);
    if (status) {
 ...
 */
async function handleTaskStats(tools) {
