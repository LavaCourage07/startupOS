    ...
    for (const type of types) {
        if (lowerInput.includes(type.toLowerCase())) {
            ...
            if (type === '任务' || type === 'task')
                return 'Task';
            if (type === '人员' || type === 'person')
                return 'Person';
            if (type === '目标' || type === 'goal')
                return 'Goal';
        }
    }
    return null;
 ...
 */
async function handleQuery(params, tools) {
 ...
 */
async function handleCreate(params, tools) {
 ...
 */
async function handleUpdate(params, tools) {
    const instruction = params['instruction'] || '';
 ...
 */
async function handleDelete(params, _tools) {
    const instruction = params['instruction'] || '';
    // Extract entity ID from instruction
    const idMatch = instruction.match(/[a-z]+_[a-z0-9]+/i);
    if (!idMatch) {
 ...
 */
async function handleValidate(tools) {
