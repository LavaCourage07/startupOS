 ...
 */
function extractPersonName(input) {
...
async function handle(context) {
    ...
    const query = input?.data || input?.message || '';
    if (!query) {
        return {
            success: false,
            error: {
 ...
 */
async function handleCountQuery(query, tools) {
 ...
 */
async function handlePersonQuery(personName, query, tools) {
    ...
    if (lower.includes('任务') || lower.includes('task')) {
        ...
        const personTasks = allTasks.filter((t) => t.properties.assignee === personName ||
 ...
 */
async function handleStatusQuery(query, tools) {
    const lower = query.toLowerCase();
    let status = null;
 ...
 */
async function handleGeneralQuery(query, tools) {
    const entityType = extractEntityType(query);
    if (!entityType) {
