...
const skills_1 = require("../../../../lib/integrations/pi-agent/core/skills");
 ...
 */
function buildSkillToolsSection() {
    ...
    const tools = (0, registry_1.getToolRegistry)().getEnabled();
    const grouped = {};
 ...
 */
function buildSkillSystemPrompt(skillName, skillContent, baseDir, dependencies, prerequisites, outputDir) {
    const lines = [];
...
function resolveInheritedMemory(agentWorkingDir) {
    const readOptional = (fileName) => {
        const filePath = path_1.default.join(agentWorkingDir, fileName);
        if (!(0, fs_1.existsSync)(filePath)) {
            return undefined;
        }
        try {
            const content = (0, fs_1.readFileSync)(filePath, 'utf-8').trim();
...
function injectInheritedMemory(prompt, inheritedMemory) {
...
function findSkillFile(skillCode) {
    ...
    for (const base of skillDirs) {
        if (!(0, fs_1.existsSync)(base))
            continue;
        try {
            const entries = (0, fs_1.readdirSync)(base, { withFileTypes: true });
            for (const entry of entries) {
                if (entry.isDirectory()) {
                    const skillMd = path_1.default.join(base, entry.name, 'SKILL.md');
                    if ((0, fs_1.existsSync)(skillMd)) {
                        ...
                        if (code === skillCode || name === skillCode) {
                            return { skillMdPath: skillMd, baseDir: path_1.default.join(base, entry.name) };
                        }
                    }
                }
            }
...
class SkillLauncher extends base_1.Launcher {
    constructor() {
        super(...arguments);
        this.entryType = 'skill';
    }
    async launch(ctx) {
        try {
            // 1. 查找并读取 SKILL.md
            ...
            const skillMd = (0, fs_1.readFileSync)(skillInfo.skillMdPath, 'utf-8');
            ...
            if (dependencies.length > 0 || prerequisites.length > 0) {
                console.log(`[SkillLauncher] Skill "${ctx.entryId}" has dependencies:`, [...dependencies, ...prerequisites]);
            }
            // 3. 构建系统提示词（含依赖安装指引）
            ...
            const dataSkillsDir = path_1.default.join((0, paths_1.getDataRoot)(), 'skills', ctx.entryId);
            ...
            console.log('[SkillLauncher] resolved paths:', {
                dataRoot: (0, paths_1.getDataRoot)(),
                agentWorkingDir,
                frontmatterOutputDir,
                resolvedOutputDir,
            });
            const systemPrompt = buildSkillSystemPrompt(ctx.entryId, skillMd, agentWorkingDir, dependencies, prerequisites, resolvedOutputDir);
...
exports.SkillLauncher = SkillLauncher;
