...
exports.ontologyDataTools = void 0;
const typebox_1 = require("@sinclair/typebox");
const store = __importStar(require("../../../../lib/features/ontology-data-store/store"));
const queryEngine = __importStar(require("../../../../lib/features/ontology-data-store/query-engine"));
const schemaValidator = __importStar(require("../../../../lib/features/ontology-data-store/schema-validator"));
const instance_relations_1 = require("../../../../lib/features/ontology-data-store/instance-relations");
...
function successResult(data) {
    logToolEnd({ toolCallId: "n/a", toolName: "n/a" }, data);
    return {
...
// 工具: create_instance
// ============================================================================
const CreateInstanceParamsSchema = typebox_1.Type.Object({
    ...
    conceptId: typebox_1.Type.String({ minLength: 1, description: "所属概念的 ID。如不确定可用值，先调用 list_concepts 或 search_ontology 获取，不要自己生成。" }),
    fields: typebox_1.Type.Object({}, { additionalProperties: true, description: "实例的字段数据，结构由概念定义决定。如不确定字段结构，先调用 get_concept_schema 查询字段约束。" }),
    createdBy: typebox_1.Type.Union([typebox_1.Type.Literal("user"), typebox_1.Type.Literal("agent"), typebox_1.Type.Literal("skill")], { optional: true, description: "创建来源，默认 agent" }),
});
const CreateInstanceTool = {
    ...
    async execute(toolCallId, params, signal, onUpdate) {
        ...
        try {
            ...
            const instance = await store.createInstance(params.ontologyId, params.conceptId, plainFields, params.createdBy ?? "agent");
            sendProgress(ctx, "实例创建完成，处理关系字段...", 0.7);
            // 自动建立实例关系
            const createdRelations = [];
            if (relationFields.size > 0) {
                for (const [fieldName, relDef] of relationFields) {
                    const rawValue = params.fields[fieldName];
                    const targetIds = Array.isArray(rawValue)
                        ? rawValue.map(String)
                    ...
                    for (const targetInstanceId of targetIds) {
                        try {
                            const relation = await (0, instance_relations_1.createInstanceRelation)(params.ontologyId, {
...
// 工具: get_instance
// ============================================================================
const GetInstanceParamsSchema = typebox_1.Type.Object({
    ontologyId: typebox_1.Type.String({ minLength: 1, description: "项目的本体 ID，形如 `ontology-{projectId}`。从 projectContext 或【协作上下文】获取，不要自己生成。" }),
    conceptId: typebox_1.Type.String({ minLength: 1, description: "所属概念的 ID。如不确定可用值，先调用 list_concepts 获取，不要自己生成。" }),
    instanceId: typebox_1.Type.String({ minLength: 1, description: "实例 ID。从 query_instances 或 create_instance 的返回中获取，不要自己生成。" }),
});
const GetInstanceTool = {
    name: "get_instance",
    label: "获取实例（数据层）",
...
// 工具: update_instance
// ============================================================================
const UpdateInstanceParamsSchema = typebox_1.Type.Object({
    ontologyId: typebox_1.Type.String({ minLength: 1, description: "项目的本体 ID，形如 `ontology-{projectId}`。从 projectContext 或【协作上下文】获取，不要自己生成。" }),
    conceptId: typebox_1.Type.String({ minLength: 1, description: "所属概念的 ID。如不确定可用值，先调用 list_concepts 获取，不要自己生成。" }),
    instanceId: typebox_1.Type.String({ minLength: 1, description: "要更新的实例 ID。从 query_instances 或 create_instance 的返回中获取，不要自己生成。" }),
    fields: typebox_1.Type.Object({}, { additionalProperties: true, description: "需要更新的字段，只需传入有变动的字段（partial update），不传的字段保持不变。" }),
});
const UpdateInstanceTool = {
    ...
    async execute(toolCallId, params, signal, onUpdate) {
        ...
        try {
            ...
            const updated = await store.updateInstance(params.ontologyId, params.conceptId, params.instanceId, plainFields);
...
// 工具: delete_instance
// ============================================================================
const DeleteInstanceParamsSchema = typebox_1.Type.Object({
    ontologyId: typebox_1.Type.String({ minLength: 1, description: "项目的本体 ID，形如 `ontology-{projectId}`。从 projectContext 或【协作上下文】获取，不要自己生成。" }),
    conceptId: typebox_1.Type.String({ minLength: 1, description: "所属概念的 ID。如不确定可用值，先调用 list_concepts 获取，不要自己生成。" }),
    instanceId: typebox_1.Type.String({ minLength: 1, description: "要删除的实例 ID。操作不可逆，确认无误再执行。" }),
});
const DeleteInstanceTool = {
...
// 工具: query_instances
// ============================================================================
const QueryInstancesParamsSchema = typebox_1.Type.Object({
    ontologyId: typebox_1.Type.String({ minLength: 1, description: "项目的本体 ID，形如 `ontology-{projectId}`。从 projectContext 或【协作上下文】获取，不要自己生成。" }),
    conceptId: typebox_1.Type.String({ minLength: 1, description: "要查询的概念 ID。如不确定可用值，先调用 list_concepts 获取，不要自己生成。" }),
    filters: typebox_1.Type.Object({}, { additionalProperties: true, optional: true, description: "按字段过滤，例如 { status: 'active' }。字段名对应实例的 fields 对象的键。" }),
    page: typebox_1.Type.Number({ optional: true, description: "分页页码，1-based，默认 1" }),
    limit: typebox_1.Type.Number({ optional: true, description: "每页数量，默认 20，最大 100" }),
    sortBy: typebox_1.Type.String({ optional: true, description: "排序字段名，对应 fields 中的键" }),
    sortOrder: typebox_1.Type.Union([typebox_1.Type.Literal("asc"), typebox_1.Type.Literal("desc")], { optional: true, description: "排序方向：asc 升序 / desc 降序，默认 asc" }),
});
const QueryInstancesTool = {
    ...
    async execute(toolCallId, params, signal, onUpdate) {
        ...
        try {
            ...
            const result = await queryEngine.queryInstances(params.ontologyId, params.conceptId, {
            ...
            let allRelations = [];
            try {
                const relationData = await (0, instance_relations_1.listInstanceRelations)(params.ontologyId);
            ...
            const itemsWithRelations = result.items.map((item) => ({
                ...item,
                relations: allRelations.filter((r) => instanceIds.has(r.sourceInstanceId) && r.sourceInstanceId === item.id
...
// ============================================================================
const GetConceptSchemaParamsSchema = typebox_1.Type.Object({
    ontologyId: typebox_1.Type.String({ minLength: 1, description: "项目的本体 ID，形如 `ontology-{projectId}`。从 projectContext 或【协作上下文】获取，不要自己生成。" }),
    conceptId: typebox_1.Type.String({ minLength: 1, description: "概念的 ID。如不确定可用值，先调用 list_concepts 获取，不要自己生成。" }),
});
const GetConceptSchemaTool = {
...
// 工具: list_concepts
// ============================================================================
const ListConceptsParamsSchema = typebox_1.Type.Object({
    ontologyId: typebox_1.Type.String({ minLength: 1, description: "项目的本体 ID，形如 `ontology-{projectId}`。从 projectContext 或【协作上下文】获取，不要自己生成。" }),
});
const ListConceptsTool = {
    name: "list_concepts",
    label: "列出概念（结构层）",
    ...
    async execute(toolCallId, params, signal, onUpdate) {
        ...
        try {
            logToolStart(ctx, params);
            checkAbort(ctx.signal);
            const concepts = await Promise.resolve().then(() => __importStar(require("../../../../lib/features/ontology-data-store/ontology-ops"))).then((m) => m.listConcepts(params.ontologyId));
