...
exports.assembleProjectPrompt = assembleProjectPrompt;
exports.buildProjectPromptLayers = buildProjectPromptLayers;
exports.rebuildProjectToolboxLayer = rebuildProjectToolboxLayer;
...
function assembleProjectPrompt(layers) {
...
function buildProjectPromptLayers(ctx) {
    return {
...
function rebuildProjectToolboxLayer(ctx) {
...
function buildLayer2_StateMemory(ctx) {
    const businessModelPath = path_1.default.join(ctx.workingDirectory, 'output', 'business-model.json');
    const hasBusinessModel = (0, fs_1.existsSync)(businessModelPath);
...
function buildPatternsLazySection(patternsMd) {
    // 只提取标题作为索引，不注入全文
...
function buildKnowledgeLazySection(knowledgeMd) {
...
function buildLayer4_Toolbox(_ctx) {
    return `## Toolbox
...
function buildWorkflowSkillsSection() {
...
function buildSystemToolsSection() {
    ...
    const toolGroups = (0, registry_1.getEnabledToolsByCategory)('project');
