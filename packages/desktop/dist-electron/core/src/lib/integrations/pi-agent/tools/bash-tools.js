...
exports.__test__ = exports.bashTools = void 0;
...
function findExecutableOnPath(name) {
    for (const pathDir of (process.env["PATH"] || "").split(path_1.delimiter)) {
        if (!pathDir)
            continue;
        const candidate = (0, path_1.join)(pathDir, name);
...
function findWindowsShell() {
    ...
    if (pwsh)
        return pwsh;
    const powershell = findExecutableOnPath("powershell.exe") ?? findExecutableOnPath("powershell");
    if (powershell)
        return powershell;
 ...
 */
function resolveShellFromEnv(value) {
    if (!value)
        return null;
...
function parseShellCandidates(value) {
    return value
        .split(/[\s,;]+/)
        .map((candidate) => candidate.trim().replace(/^["']|["']$/g, ""))
        .filter(Boolean);
}
function resolveShellCandidate(candidate) {
    ...
    for (const pathDir of (process.env["PATH"] || "").split(path_1.delimiter)) {
        if (!pathDir)
            continue;
        const shellPath = (0, path_1.join)(pathDir, candidateName);
        if (isExecutable(shellPath)) {
            return shellPath;
        }
    }
...
function isExecutable(shellPath) {
    try {
        (0, fs_1.accessSync)(shellPath, fs_1.constants.X_OK);
        return true;
    }
    catch {
        return false;
 ...
 * 检查环境变量是否是支持的 shell
 */
function isEnvShellSupported(envShell) {
 ...
 * 工具层不关心来源，只使用该值
 */
async function resolveWorkingDirectory(paramsWorkingDirectory) {
    ...
    const effectiveDir = (0, context_1.getToolContext)().workingDirectory;
    if (effectiveDir) {
        return effectiveDir;
    }
    ...
    if (paramsWorkingDirectory) {
        ...
        if (path_1.win32.isAbsolute(paramsWorkingDirectory) || paramsWorkingDirectory.startsWith("/")) {
            return paramsWorkingDirectory;
        }
        return `${(0, paths_1.getDataRoot)()}/${paramsWorkingDirectory}`;
    }
    // 回退到可写的数据目录（打包后为 ~/Library/Application Support/.../data）
    return (0, paths_1.getDataRoot)();
}
function createToolCtx(toolCallId, toolName, signal, onUpdate) {
...
// 工具: 执行命令
// ============================================================================
const ExecuteCommandParamsSchema = typebox_1.Type.Object({
...
const ExecuteCommandTool = {
    ...
    label: "执行命令",
    description: "执行 shell 命令并返回结果。注意：出于安全考虑，某些危险命令会被拒绝执行。",
    ...
    async execute(toolCallId, params, signal, onUpdate) {
        ...
        try {
            ...
            if (!shellPath) {
                throw new Error("No suitable shell found. Please ensure bash or zsh is installed.");
            }
            // 设置超时
