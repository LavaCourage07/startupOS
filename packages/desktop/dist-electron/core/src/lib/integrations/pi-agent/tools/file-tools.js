...
function sendProgress(ctx, message, progress) {
...
// ============================================================================
const ReadFileParamsSchema = typebox_1.Type.Object({
    filePath: typebox_1.Type.String({ minLength: 1, description: "文件路径，相对于工作目录（system prompt 中已告知）。不要拼接 `data/projects/...` 等绝对路径，直接写相对路径如 `output/report.md`。" }),
    offset: typebox_1.Type.Optional(typebox_1.Type.Number({ description: "从第几行开始读取（1-based），用于读取大文件的特定部分" })),
    limit: typebox_1.Type.Optional(typebox_1.Type.Number({ description: "最多读取多少行，与 offset 配合使用" })),
});
const ReadFileTool = {
    name: "read_file",
    label: "读取文件",
    ...
    async execute(toolCallId, params, signal, onUpdate) {
        ...
        try {
            ...
            try {
                ...
                try {
                    const entries = await fs_1.promises.readdir(parentDir, { withFileTypes: true });
            ...
            const rawContent = await fs_1.promises.readFile(fullPath, "utf-8");
            ...
            const errorResult = {
                ...
                filePath: (0, path_utils_1.normalizeToolDisplayPath)(params.filePath),
            };
            return {
                details: errorResult,
                content: [{
...
// ============================================================================
const WriteFileParamsSchema = typebox_1.Type.Object({
    filePath: typebox_1.Type.String({ minLength: 1, description: "文件路径，相对于工作目录。不要拼接绝对路径。不存在的目录会自动创建。" }),
    content: typebox_1.Type.String({ description: "要写入的完整内容。注意：会完整覆盖原文件，如需追加请先 read_file 再 write_file。" }),
});
const WriteFileTool = {
    name: "write_file",
    label: "写入文件",
    ...
    async execute(toolCallId, params, signal, onUpdate) {
        ...
        try {
            ...
            sendProgress(ctx, `准备目录: ${path_1.default.dirname(displayPath)}`, 0.2);
            ...
            await fs_1.promises.writeFile(fullPath, params.content, "utf-8");
            checkAbort(ctx.signal);
            // 文件内容校验（按类型验证格式正确性）
...
// ============================================================================
const EditFileParamsSchema = typebox_1.Type.Object({
    filePath: typebox_1.Type.String({ minLength: 1, description: "文件路径，相对于工作目录。不要拼接绝对路径。" }),
    oldString: typebox_1.Type.String({ minLength: 1, description: "要查找的字符串，必须是文件内唯一存在的子串。如果有多处相同内容，需提供更多上下文或设置 replaceAll=true。" }),
    newString: typebox_1.Type.String({ description: "替换后的字符串。可以为空字符串（表示删除 oldString）。" }),
    replaceAll: typebox_1.Type.Optional(typebox_1.Type.Boolean({ description: "是否替换所有匹配项，默认 false（只替换第一处且要求唯一）" })),
});
const EditFileTool = {
    name: "edit_file",
    label: "编辑文件",
    ...
    async execute(toolCallId, params, signal, onUpdate) {
        ...
        try {
            ...
            let originalContent;
            try {
                originalContent = await fs_1.promises.readFile(fullPath, "utf-8");
            }
            catch (error) {
            ...
            if (matches > 1 && !params.replaceAll) {
                const preview = params.oldString.length > 100
                    ? params.oldString.substring(0, 100) + '...'
                    : params.oldString;
                throw new Error(`Found ${matches} matches of the string to replace, but replaceAll is false. ` +
...
// ============================================================================
const ListFilesParamsSchema = typebox_1.Type.Object({
    directory: typebox_1.Type.Optional(typebox_1.Type.String({ default: "." })),
    recursive: typebox_1.Type.Optional(typebox_1.Type.Boolean({ description: "是否递归列出子目录，默认 false" })),
});
const ListFilesTool = {
    name: "list_files",
    label: "列出文件",
    ...
    async execute(toolCallId, params, signal, onUpdate) {
        ...
        try {
            ...
            let realBoundary;
            try {
                realBoundary = await fs_1.promises.realpath(boundary);
            }
            catch {
            ...
            async function readDirRecursive(dir, base) {
                ...
                for (const entry of entries) {
                    const entryAbs = path_1.default.join(dir, entry.name);
                    let realEntry;
                    try {
                        realEntry = await fs_1.promises.realpath(entryAbs);
                    }
                    catch {
                        continue; // 损坏的 symlink 或权限不足，跳过
                    }
                    if (realEntry !== realBoundary && !realEntry.startsWith(realBoundary + path_1.default.sep)) {
                        continue; // entry 通过 symlink 越界，跳过
                    }
                    const relativePath = (0, path_utils_1.joinToolDisplayPath)(base, entry.name);
            ...
            const errorResult = {
                success: false,
                error: error instanceof Error ? error.message : String(error),
                directory: (0, path_utils_1.normalizeToolDisplayPath)(params.directory || "."),
            };
            return {
                details: errorResult,
                content: [{
...
// 工具: 删除文件
// ============================================================================
const DeleteFileParamsSchema = typebox_1.Type.Object({
    filePath: typebox_1.Type.String({ minLength: 1, description: "文件或目录路径，相对于工作目录。注意：目录会被递归删除，操作不可逆。" }),
});
    ...
    async execute(toolCallId, params, signal, onUpdate) {
        ...
        try {
            ...
            if (stats.isDirectory()) {
                await fs_1.promises.rm(fullPath, { recursive: true });
            }
            else {
                await fs_1.promises.unlink(fullPath);
            }
