...
exports.extractAndIngest = extractAndIngest;
...
function classify(data) {
    ...
    if (!data.outcome.resolved || corrections > 0 || anyError)
        return 'negative';
    return 'positive';
}
function buildPositiveText(payload) {
    ...
    if (summary)
        parts.push(`结果摘要: ${summary}`);
    return parts.join('\n');
}
function buildNegativeText(payload) {
    const scene = summarizeScene(payload.scene, payload.toolChain);
    const parts = [`[NEGATIVE] ${scene}`];
    parts.push(`路径: ${payload.toolChain.join(' → ')}`);
    const failureReason = summarizeToolResult(payload.failureReason);
    if (failureReason)
        parts.push(`失败原因: ${failureReason}`);
    if (payload.userFeedback)
        parts.push(`用户反馈: "${payload.userFeedback}"`);
    parts.push(`教训: 避免在类似场景使用该路径`);
    return parts.join('\n');
}
function summarizeScene(scene, toolChain) {
    const cleaned = compactText(scene);
    if (cleaned)
        return `场景: ${cleaned.slice(0, 120)}`;
...
async function extractAndIngest(data, correctionSignals, archival) {
    ...
    const strength = (0, correction_detector_1.maxStrength)(correctionSignals);
    const payload = {
        polarity,
