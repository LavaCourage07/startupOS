...
exports.renderMemoryBlocksXML = void 0;
exports.assemblePrompt = assemblePrompt;
exports.buildRoleSystemPrompt = buildRoleSystemPrompt;
exports.buildPromptLayers = buildPromptLayers;
exports.rebuildToolboxLayer = rebuildToolboxLayer;
exports.rebuildStateMemoryLayer = rebuildStateMemoryLayer;
exports.buildSkillMarkdown = buildSkillMarkdown;
const base_1 = require("../../../../lib/features/services/launcher/base");
const registry_1 = require("../../../../lib/integrations/pi-agent/tools/registry");
const memory_consumption_1 = require("../memory-consumption");
Object.defineProperty(exports, "renderMemoryBlocksXML", { enumerable: true, get: function () { return memory_consumption_1.renderMemoryBlocksXML; } });
...
function assemblePrompt(layers) {
...
// 公开 API
// ============================================================================
function buildRoleSystemPrompt(roleContext, stateMachine) {
    return assemblePrompt(buildPromptLayers(roleContext, stateMachine));
}
function buildPromptLayers(roleContext, stateMachine) {
    return {
...
function rebuildToolboxLayer(roleContext) {
    return buildLayer4_Toolbox(roleContext);
}
/** 只重建状态记忆层（Memory.md 或阶段变化时调用） */
function rebuildStateMemoryLayer(roleContext, stateMachine) {
    return buildLayer2_StateMemory(roleContext, stateMachine);
}
// ============================================================================
// 各层构建函数
// ============================================================================
function buildLayer1_Identity(ctx) {
    return `## Role Identity\n\n${ctx.agentMd}`;
}
function buildLayer2_StateMemory(ctx, sm) {
    const phaseSection = (() => {
...
function buildLayer3_ThinkingLoop() {
    return `\
...
function buildLayer4_Toolbox(ctx) {
    return `## Toolbox
...
function buildInstalledSkillsSection(ctx) {
...
function buildSkillManagementSection() {
    return `\
### Skill Management
...
function buildSystemToolsSection() {
    ...
    const toolGroups = (0, registry_1.getEnabledToolsByCategory)('role-agent');
    const groupBlocks = Object.entries(toolGroups)
...
function buildLayer5_Style(ctx) {
    if (!ctx.tasteMd)
        return '';
    return `## Style Guide\n\n${ctx.tasteMd}`;
}
function buildLayer6_Permissions(ctx) {
...
${base_1.AGENT_PERMISSION_PROMPT}`;
}
function buildLayer7_Safety() {
    return `## Safety
...
function buildSkillTable(skills) {
