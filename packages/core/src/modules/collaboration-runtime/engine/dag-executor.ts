...
import { CostController } from "../observability/cost-controller";
import type {
...
export interface DagExecutorConfig {
  timeoutMs?: number;        // 总执行超时（默认 5 分钟）
  maxIterations?: number;    // 最大迭代次数（默认 100）
  backPressureThreshold?: number; // 队列积压阈值（默认 10）
  agingIntervalMs?: number;  // aging 检查间隔（默认 1 秒）
  agingPriorityBoost?: number; // 每次 aging 优先级提升量（默认 1）
...
interface ResolvedDagExecutorConfig {
  timeoutMs: number;
  maxIterations: number;
  backPressureThreshold: number;
  agingIntervalMs: number;
  agingPriorityBoost: number;
  blackboard?: Blackboard;
  conflictDetector?: ConflictDetector;
  costController?: CostController;
  eventEmitter?: { emit(event: RuntimeEvent): void };
...
interface DagNode {
  ...
  queuedAt: number;       // 入队时间戳（用于 aging）
}

export interface DagResult {
  status: "completed" | "timed_out" | "failed" | "aborted" | "back_pressure";
...
type AgentExecutor = (
  agentId: string
) => Promise<{ status: "completed" | "failed" | "waiting"; output?: unknown; reviewRequest?: { question: string; context?: Record<string, unknown> } }>;
...
export class DagExecutor {
  private config: ResolvedDagExecutorConfig;
  private blackboard?: Blackboard;
  private conflictDetector?: ConflictDetector;
  private costController?: CostController;
  private topology: CollaborationTopology | null = null;
  ...
  pauseAtNode(agentId: string, reviewRequest: { question: string; context?: Record<string, unknown> }): void {
  ...
  /** 恢复指定节点（用户确认后） */
  resumeNode(agentId: string, userResponse: string): void {
    const node = this.nodes.get(agentId);
    if (node === undefined || node.status !== "waiting") { return; }
    node.status = "ready";
  ...
  private maybeMarkReady(agentId: string): void {
    const node = this.nodes.get(agentId);
  ...
  private writeNodeInput(agentId: string): void {
  ...
  private writeNodeOutput(agentId: string, output: unknown): void {
    if (!this.blackboard) {
      return;
    }
  ...
  private async dispatchNotifyEdges(agentId: string, output: unknown): Promise<void> {
    if (this.topology === null) {
      return;
    }
    const notifyEdges = this.topology.edges.filter((edge) => edge.from === agentId && edge.type === "notify");
    await Promise.all(
      notifyEdges.map(async (edge) => {
        if (this.blackboard) {
  ...
  private serializeEdge(edge: CollaborationEdge): Record<string, string> {
    return {
      from: edge.from,
      to: edge.to,
      type: edge.type,
  ...
  getSnapshot(): {
    agents: Array<{ agentId: string; status: DagNode["status"] }>;
    completedAgents: string[];
    failedAgents: string[];
    currentAgentIds: string[]; // 正在运行的 Agent
    waitingAgentIds: string[]; // 等待用户确认的 Agent
    pendingAgentIds: string[];  // 等待中的 Agent
    progress: string;           // 人类可读的进度描述
  } {
  ...
  private hasUnfinishedNodes(): boolean {
    for (const node of this.nodes.values()) {
  ...
  private getBlockedNodeIds(): string[] {
  ...
  private countByStatus(status: DagNode["status"]): number {
    let count = 0;
    for (const node of this.nodes.values()) {
  ...
  /** DAG 进度事件 — 包含全量节点状态快照 */
  private async emitProgressEvent(): Promise<void> {
    const snapshot = this.getSnapshot();
    await this.emitEvent("DAG_PROGRESS", snapshot as unknown as Record<string, unknown>, "system");
  }
  ...
  private buildResult(status: DagResult["status"]): DagResult {
    return {
      status,
      completedAgents: this.getCompletedIds(),
      failedAgents: this.getFailedIds(),
