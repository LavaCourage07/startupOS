...
import type { Blackboard, TaskItem } from "../session/types";

interface BlackboardDetailProps {
  state: Blackboard | null;
}

export function BlackboardDetail({ state }: BlackboardDetailProps) {
  const [activeTab, setActiveTab] = useState<
    "data" | "tasks" | "messages" | "locks" | "artifacts"
  >("data");
  ...
  const tabs = [
    { id: "data" as const, label: `数据 (${Object.keys(state.sharedData).length})` },
    { id: "tasks" as const, label: `任务 (${state.tasks.length})` },
    { id: "messages" as const, label: `消息 (${state.messages.length})` },
    { id: "locks" as const, label: `锁 (${Object.keys(state.locks).length})` },
    { id: "artifacts" as const, label: `产物 (${Object.keys(state.artifacts).length})` },
  ];
...
function DataView({ data }: { data: Record<string, { value: unknown; provenance?: { writer: string; version: number; timestamp: string } } | null>, }) {
  ...
  return (
    <div className="divide-y divide-gray-100">
      {keys.map((key) => {
        ...
        return (
          <div key={key} className="p-2 hover:bg-gray-50">
      ...
      {tasks.map((task) => (
        <div key={task.id} className="p-2 hover:bg-gray-50">
          <div className="flex items-center justify-between">
            ...
            <span
              className={`px-2 py-0.5 rounded text-[10px] ${STATUS_COLORS[task.status] ?? "bg-gray-100 text-gray-600"}`}
            >
              {task.status}
            </span>
          </div>
...
function MessageView({ messages }: { messages: Array<{ id: string; from: string; to: string; type: string; content: unknown; seq?: number }> }) {
  ...
  return (
    <div className="divide-y divide-gray-100">
      {messages.map((msg) => (
        <div key={msg.id} className="p-2 hover:bg-gray-50">
...
function LockView({ locks }: { locks: Record<string, { holder: string; expiresAt: string }> }) {
  ...
  return (
    <div className="divide-y divide-gray-100">
      {Object.entries(locks).map(([key, lock]) => (
        <div key={key} className="p-2 hover:bg-gray-50 flex items-center justify-between">
          <div>
            <span className="font-semibold text-gray-800">{key}</span>
...
function ArtifactView({ artifacts }: { artifacts: Record<string, { name: string; producer: string; data: unknown }> }) {
  ...
  return (
    <div className="divide-y divide-gray-100">
      {Object.values(artifacts).map((artifact) => (
        <div key={artifact.name} className="p-2 hover:bg-gray-50">
          <div className="flex items-center justify-between">
