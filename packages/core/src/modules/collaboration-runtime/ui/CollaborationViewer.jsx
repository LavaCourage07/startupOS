...
function extractAgentMessages(events, agentNames, filterAgentId) {
    const messages = [];
...
function collectAgentNames(events, names) {
    for (const event of events) {
        if (event.type === "AGENT_REGISTERED" && typeof event.payload?.["name"] === "string") {
            names[event.source] = event.payload["name"];
        }
    }
    return names;
}
...
function JsonValue({ value, depth = 0 }) {
    ...
    if (typeof value === "string") {
        if (value.length > 200) {
            return (<LongStringValue value={value}/>);
        }
    ...
    if (Array.isArray(value)) {
        ...
        return (<span>
        <button onClick={() => { setExpanded((e) => !e); }} className="text-slate-500 hover:text-slate-700 font-mono text-[10px]">
          [{expanded ? "▾" : "▸"} {value.length}]
        </button>
        {expanded && (<div className="ml-3 border-l border-slate-200 pl-2 space-y-0.5 mt-0.5">
            {value.map((item, i) => (<div key={i} className="flex gap-1">
    ...
    if (typeof value === "object") {
        ...
        {expanded && (<div className="ml-3 border-l border-slate-200 pl-2 space-y-0.5 mt-0.5">
            {entries.map(([k, v]) => (<div key={k} className="flex gap-1 flex-wrap">
...
function LongStringValue({ value }) {
    ...
    return (<span>
      ...
      <button onClick={() => { setExpanded((e) => !e); }} className="ml-1 text-[10px] text-sky-600 hover:underline">
...
// Message bubble
// ============================================================================
function MessageBubble({ msg, showAgent, onAnswer, answeredIds }) {
    ...
    if (msg.type === "thinking") {
        return (<div className="flex items-start gap-2 opacity-70">
        <span className="text-base mt-0.5">💭</span>
        <div className="text-[11px] text-slate-500 italic leading-5">{msg.text}</div>
      </div>);
    }
    if (msg.type === "tool_call") {
        return (<div className="rounded-xl border border-yellow-200 bg-yellow-50 px-3 py-2">
        ...
        {msg.toolArgs !== undefined && msg.toolArgs !== null && (<div className="mt-1.5 text-[11px] font-mono">
    ...
    if (msg.type === "tool_result") {
        ...
        if (msg.toolName === "ask_user_question") {
            ...
            if (parsed) {
                ...
                return (<div className="rounded-xl border border-blue-200 bg-blue-50 px-3 py-3">
            ...
            {displayText && (<p className="text-xs text-blue-900 mb-2 whitespace-pre-wrap">{displayText}</p>)}
            <AskUserQuestionComponent parsedQuestion={parsed} onAnswer={(labels) => { onAnswer?.(msg.id, labels); }} disabled={isAnswered || !onAnswer}/>
            {isAnswered && (<p className="mt-2 text-[10px] text-blue-500">已提交回答</p>)}
          </div>);
            }
        }
        return (<div className={`rounded-xl border px-3 py-2 ${msg.isError ? "border-rose-200 bg-rose-50" : "border-slate-200 bg-slate-50"}`}>
        <button className="flex w-full items-center gap-2 text-left" onClick={() => { setToolOpen((o) => !o); }}>
        ...
        {toolOpen && (<div className="mt-2 text-[11px] font-mono max-h-48 overflow-y-auto">
            <JsonValue value={msg.toolResult} depth={0}/>
          </div>)}
      </div>);
    }
    if (msg.type === "status") {
        return (<div className={`flex items-start gap-2 rounded-xl border px-3 py-2 ${msg.isError ? "border-rose-200 bg-rose-50" : "border-green-200 bg-green-50"}`}>
    ...
    return (<div className="flex items-start gap-3">
      {showAgent && (<span className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-xl text-[10px] font-bold text-white bg-gradient-to-br from-orange-400 to-rose-400 shadow-sm">
      ...
      <div className={`min-w-0 flex-1 rounded-[16px] border border-white/75 bg-white/90 px-4 py-3 shadow-sm ${showAgent ? "" : "ml-10"}`}>
        {showAgent && (<div className="flex items-baseline gap-2 mb-1">
        ...
        {parsedQuestion && onAnswer && (<div className="mt-2">
            <AskUserQuestionComponent parsedQuestion={parsedQuestion} onAnswer={(labels) => { onAnswer(msg.id, labels); }} disabled={isAnswered}/>
            {isAnswered && (<p className="mt-2 text-[10px] text-slate-400">已提交回答</p>)}
          </div>)}
      </div>
    </div>);
}
...
// ============================================================================
function AgentCuiPanel({ agentId, agentName, sessionId, onBack }) {
    const events = useCollaborationUi((state) => state.events);
    const agentNames = useMemo(() => collectAgentNames(events, {}), [events]);
    const messages = useMemo(() => extractAgentMessages(events, agentNames, agentId), [events, agentNames, agentId]);
    const bottomRef = useRef(null);
    const [answeredIds, setAnsweredIds] = useState(new Set());
    const handleAnswer = useCallback(async (msgId, selectedLabels) => {
    ...
    return (<div className="flex min-w-0 flex-1 flex-col overflow-hidden">
      <div className="flex flex-shrink-0 items-center gap-3 border-b border-slate-200/80 bg-white/80 px-4 py-3 backdrop-blur">
        <button onClick={onBack} className="flex items-center gap-1 rounded-lg px-2 py-1 text-xs text-slate-500 hover:bg-slate-100 transition-colors">
          ← 全部
        </button>
      ...
      <div className="flex-1 space-y-3 overflow-y-auto p-4">
        {messages.length === 0 ? (<div className="py-16 text-center text-xs text-slate-400">等待 {agentName} 的消息...</div>) : (messages.map((msg) => (<MessageBubble key={msg.id} msg={msg} showAgent={false} onAnswer={handleAnswer} answeredIds={answeredIds}/>)))}
        <div ref={bottomRef}/>
      </div>
    </div>);
}
...
// ============================================================================
function MessageStream({ sessionId, onSelectAgent }) {
    const events = useCollaborationUi((state) => state.events);
    const agentNames = useMemo(() => collectAgentNames(events, {}), [events]);
    ...
    return (<section className="flex min-w-0 flex-1 flex-col overflow-hidden bg-white/35">
      ...
      <div className="flex-1 space-y-3 overflow-y-auto bg-[linear-gradient(180deg,rgba(255,255,255,0.52),rgba(248,250,252,0.9))] px-5 py-4">
        {messages.length === 0 ? (<div className="py-16 text-center">
            <p className="text-xs text-slate-400">等待 Agent 消息...</p>
          </div>) : (messages.map((msg) => (<div key={msg.id} onClick={() => { if (msg.type !== "tool_result" || msg.toolName !== "ask_user_question")
            onSelectAgent(msg.agentId); }} className={msg.type === "tool_result" && msg.toolName === "ask_user_question" ? "" : "cursor-pointer"}>
...
// ============================================================================
function AgentActivityCards({ onSelect, selectedId }) {
    const activities = useCollaborationUi((state) => state.activities);
    const events = useCollaborationUi((state) => state.events);
    const agentNamesRef = useRef({});
    const visibleActivities = useMemo(() => Object.values(activities).filter((activity) => !isSupervisorAgent(activity.agentId)), [activities]);
    useEffect(() => {
        for (const event of events) {
            if (event.type === "AGENT_REGISTERED" && typeof event.payload?.["name"] === "string") {
                agentNamesRef.current[event.source] = event.payload["name"];
            }
        }
    }, [events]);
    return (<aside className="w-full flex-shrink-0 border-b border-slate-200/70 bg-white/55 p-4 backdrop-blur-xl lg:w-72 lg:border-b-0 lg:border-r">
      ...
      {visibleActivities.length === 0 ? (<div className="rounded-2xl border border-dashed border-slate-200 bg-white/65 p-5 text-center text-xs text-slate-400">
          暂无可展示的 Agent
        </div>) : (visibleActivities.map((activity) => {
            ...
            return (<button key={activity.agentId} onClick={() => { onSelect(activity.agentId); }} className={`w-full rounded-2xl border px-3.5 py-3 text-left shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md ${colors.bg} ${isSelected
              ...
              {activity.message !== undefined && (<div className="mt-2 line-clamp-2 text-[11px] leading-4 text-slate-500">
                  {activity.message}
                </div>)}
...
function ConnectionStatus() {
    ...
    return (<div className="flex items-center gap-2">
      ...
      {isConnected && (<span className="flex items-center gap-2 rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-xs font-medium text-emerald-700">
...
function BlackboardSection() {
    ...
    return (<div className="flex-shrink-0 border-t border-slate-200/70 bg-white/65 px-4 py-3 backdrop-blur-xl">
...
function HitlCard({ request, sessionId, onDismiss }) {
    ...
    return (<div className="flex-shrink-0 border-b border-amber-200 bg-amber-50 px-4 py-3">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {request.options.length > 0 ? (<div className="flex flex-wrap gap-2 mb-2">
              {request.options.map((opt) => (<button key={opt.label} onClick={() => { toggleOption(opt.label); }} className={`rounded-lg border px-3 py-1.5 text-xs transition-colors ${selected.includes(opt.label)
            ...
            </div>) : (<input type="text" value={freeText} onChange={(e) => { setFreeText(e.target.value); }} onKeyDown={(e) => { if (e.key === "Enter") {
            void handleSubmit();
        } }} placeholder="输入您的回答..." className="w-full rounded-lg border border-amber-300 bg-white px-3 py-1.5 text-xs text-slate-800 outline-none focus:border-amber-500 mb-2"/>)}
          <div className="flex gap-2">
            <button disabled={submitting || (request.options.length > 0 ? selected.length === 0 : freeText.trim() === "")} onClick={() => { void handleSubmit(); }} className="rounded-lg bg-amber-500 px-3 py-1.5 text-xs font-semibold text-white disabled:opacity-50 hover:bg-amber-600 transition-colors">
              {submitting ? "提交中..." : "确认"}
            </button>
          </div>
        </div>
...
function SupervisorPlanPanel({ events }) {
    ...
    return (<div className="flex-shrink-0 border-b border-slate-200/70 bg-slate-50/70 px-4 py-2.5 backdrop-blur-xl">
      <div className="flex items-center gap-2 flex-wrap">
        <span className="text-[10px] font-bold uppercase tracking-[0.15em] text-slate-400 flex-shrink-0">Supervisor 计划</span>
        {workers.map((w) => (<div key={w.workerId} className="flex items-center gap-1 rounded-full border border-slate-200 bg-white px-2 py-0.5 text-[10px] text-slate-600" title={w.action}>
...
export function CollaborationViewer({ sessionId, uiDeps }) {
    useSSEConnection(sessionId);
    const [selectedAgentId, setSelectedAgentId] = useState(null);
    const [dismissedHitls, setDismissedHitls] = useState(new Set());
    const events = useCollaborationUi((state) => state.events);
    const agentNames = useMemo(() => collectAgentNames(events, {}), [events]);
    ...
    const pendingHitls = useMemo(() => {
        const seen = new Set();
        const result = [];
        for (const ev of events) {
            if (ev.type !== "HUMAN_REVIEW_REQUEST")
                continue;
            if (seen.has(ev.id) || dismissedHitls.has(ev.id))
                continue;
            seen.add(ev.id);
            result.push({
                eventId: ev.id,
    ...
    <div className="flex h-full w-full flex-col overflow-hidden rounded-xl border border-white/45 bg-[linear-gradient(180deg,rgba(255,255,255,0.72),rgba(248,250,252,0.9))] shadow-[0_18px_45px_-30px_rgba(15,23,42,0.35)]">
      ...
      {/* HITL 待回答卡片（最多显示最新一条） */}
      {pendingHitls.slice(-1).map((req) => (<HitlCard key={req.eventId} request={req} sessionId={sessionId} onDismiss={() => { dismissHitl(req.eventId); }}/>))}

      {/* Main Content */}
      <div className="flex min-h-0 flex-1 flex-col lg:flex-row">
        <AgentActivityCards onSelect={(id) => { setSelectedAgentId((cur) => cur === id ? null : id); }} selectedId={selectedAgentId}/>

        {selectedAgentId !== null && selectedName !== null ? (<AgentCuiPanel agentId={selectedAgentId} agentName={selectedName} sessionId={sessionId} onBack={() => { setSelectedAgentId(null); }}/>) : (<MessageStream sessionId={sessionId} onSelectAgent={(id) => { setSelectedAgentId(id); }}/>)}
      </div>
