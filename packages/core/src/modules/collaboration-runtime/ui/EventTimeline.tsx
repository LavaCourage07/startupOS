    ...
    <div className="space-y-2 max-h-96 overflow-y-auto">
      {sortedEvents.map((event) => {
        ...
        return (
          <div
            key={event.id}
            className={`flex items-start gap-3 p-2 rounded ${colors.bg} text-xs`}
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                ...
                {event.source && (
                  <span className="text-gray-500 font-mono">[{event.source}]</span>
                )}
              </div>
              {payloadSummary && (
                <p className="text-gray-600 truncate mt-0.5">{payloadSummary}</p>
              )}
            </div>
          </div>
        );
