...
const UiDepsContext = createContext(null);
function useUiDeps() {
    const ctx = useContext(UiDepsContext);
    if (!ctx)
        throw new Error("UiDepsContext not provided");
    return ctx;
}
function formatWorkerEvents(agentId, events) {
    return events
        ...
        switch (event.type) {
            ...
            case "AGENT_ACT":
                ...
                break;
            case "TOOL_RESULT":
                text = `工具返回：${typeof event.payload?.["result"] === "string" ? event.payload["result"] : JSON.stringify(event.payload?.["result"] ?? {})}`;
                kind = "tool";
                break;
            case "TOOL_FAILURE":
...
function HitlCard({ req, resolveAgentName, onReply, disabled }) {
    const { ChatInputBar } = useUiDeps();
    const displayName = req.workerName || (req.workerId ? resolveAgentName(req.workerId) : "");
    return (<div className="flex-shrink-0 border-t border-yellow-200/80 bg-yellow-50/90">
      <ChatInputBar onSubmit={onReply} disabled={disabled} placeholder={displayName ? `回复 ${displayName} 的问题…` : "输入回复…"} lightBg className="bg-yellow-50/80"/>
    </div>);
}
function WorkerDetailDrawer({ agentId, agentName, activity, events, onClose }) {
    ...
    return (<div className="flex h-full flex-col overflow-hidden bg-white">
      ...
      <div className="flex flex-shrink-0 items-center justify-between border-b border-gray-100 px-4 py-3">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            {activity && (<div className={`text-xs font-medium ${statusColor[activity.status] ?? "text-gray-400"}`}>
                {statusLabel[activity.status] ?? activity.status}
              </div>)}
          </div>
        </div>
        <button onClick={onClose} className="flex h-7 w-7 items-center justify-center rounded-full text-gray-400 transition hover:bg-gray-100 hover:text-gray-600" aria-label="关闭">
          <X className="h-4 w-4"/>
        </button>
      </div>
      ...
      <div ref={scrollRef} className="flex-1 min-h-0 space-y-2 overflow-y-auto px-4 py-3">
        {workerEvents.length === 0 ? (<div className="flex h-20 items-center justify-center rounded-lg border border-dashed border-gray-200 text-xs text-gray-400">
            暂无活动记录
          </div>) : (workerEvents.map((item) => (<div key={item.id} className={`rounded-lg border px-3 py-2 text-xs leading-5 ${item.kind === "error"
              ...
              {item.kind === "message" ? (<MarkdownContent content={item.text}/>) : (<div className="whitespace-pre-wrap break-words">{item.text}</div>)}
            </div>)))}
      </div>
    </div>);
}
function AgentStatusBar({ topology, activities, recentlyActiveAgents, onNodeClick, }) {
    ...
    return (<div className="flex flex-shrink-0 items-center gap-2 overflow-x-auto border-b border-gray-100 bg-white px-5 py-2.5">
      <span className="flex-shrink-0 text-xs text-gray-400">内部进度</span>
      <div className="mx-1 h-3 w-px bg-gray-200"/>
      <div className="flex items-center gap-1.5">
        {agentIds.map((id) => {
            const agent = topology.agents[id];
            const activity = activities[id];
            const status = activity?.status ?? "idle";
            const isActive = recentlyActiveAgents.includes(id);
            return (<button key={id} onClick={() => onNodeClick(id)} className={`flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-xs font-medium transition ${isActive
...
const COORDINATION_STYLE = {
    dispatch: "text-blue-600",
    result: "text-green-600",
    status: "text-yellow-600",
    general: "text-gray-400",
};
export const MultiAgentLauncher = ({ projectId, projectName, uiDeps, llmConfig, }) => {
    ...
    const lastUserEventId = useMemo(() => {
        const lastUserEv = [...events].reverse().find((e) => e.source === "user" && (e.type === "USER_INPUT" || e.type === "USER_REPLY_TO_SUPERVISOR" || e.type === "HUMAN_REVIEW_RESPONSE"));
    ...
    const allDisplayMessages = useMemo(() => (pendingUserMessage ? [...displayMessages, pendingUserMessage] : displayMessages), [displayMessages, pendingUserMessage]);
    const pendingHitlRequests = useMemo(() => {
        ...
        for (const ev of events) {
            ...
            const alreadyReplied = events.some((e) => (e.type === "USER_REPLY_TO_SUPERVISOR" || e.type === "HUMAN_REVIEW_RESPONSE") &&
    ...
    useEffect(() => {
        ...
        if ((latest.type === "ASSISTANT_MESSAGE" || latest.type === "MESSAGE_SENT") &&
    ...
    const uploadBasePath = useCallback(() => (sessionId ? `data/projects/${projectId}/collaboration-sessions/${sessionId}/attachments` : null), [sessionId, projectId]);
    ...
    const submitReply = useCallback(async (message, workerId) => {
        if (!message.trim() || sessionId === null)
            return;
    ...
    const agentNames = topology !== null
        ? Object.fromEntries(Object.entries(topology.agents).map(([id, a]) => [id, a.name]))
        : {};
    ...
    const phaseBadge = (() => {
        if (isLaunching)
            return (<span className="inline-flex items-center gap-1.5 rounded-full border border-gray-200 bg-white px-2.5 py-1 text-xs font-medium text-gray-500">
        <Loader2 className="h-3 w-3 animate-spin"/>
        {phase === "creating" ? "创建中" : "启动中"}
      </span>);
        if (isGreeting)
            return (<span className="inline-flex items-center gap-1.5 rounded-full border border-blue-200 bg-blue-50 px-2.5 py-1 text-xs font-medium text-blue-600">
        <span className="h-1.5 w-1.5 rounded-full bg-blue-400"/>
        等待目标
      </span>);
        if (isRunning && !isAwaitingHitl)
            return (<span className="inline-flex items-center gap-1.5 rounded-full border border-blue-200 bg-blue-50 px-2.5 py-1 text-xs font-medium text-blue-600">
        <span className="h-1.5 w-1.5 rounded-full bg-blue-500 animate-pulse"/>
        执行中
      </span>);
        if (isRunning && isAwaitingHitl)
            return (<span className="inline-flex items-center gap-1.5 rounded-full border border-yellow-200 bg-yellow-50 px-2.5 py-1 text-xs font-medium text-yellow-600">
        <span className="h-1.5 w-1.5 rounded-full bg-yellow-400"/>
        等待回复
      </span>);
        if (phase === "completed")
            return (<span className="inline-flex items-center gap-1.5 rounded-full border border-green-200 bg-green-50 px-2.5 py-1 text-xs font-medium text-green-600">
        <span className="h-1.5 w-1.5 rounded-full bg-green-500"/>
        已完成
      </span>);
        if (phase === "error")
            return (<span className="inline-flex items-center gap-1.5 rounded-full border border-red-200 bg-red-50 px-2.5 py-1 text-xs font-medium text-red-600">
        出错
      </span>);
        return null;
    })();
    return (<UiDepsContext.Provider value={uiDeps}>
    <div className="flex h-full w-full flex-col bg-white text-gray-900">
      ...
      {/* Agent status bar */}
      {topology !== null && (isRunning || isLaunching) && (<AgentStatusBar topology={topology} activities={activities} recentlyActiveAgents={recentlyActiveAgents} onNodeClick={(id) => setSelectedAgentId((prev) => prev === id ? null : id)}/>)}

      {/* Error banner */}
      {error !== null && (<div className="mx-5 mt-3 flex-shrink-0 rounded-lg border border-red-200 bg-red-50 px-4 py-2.5 text-xs text-red-600">
          {error}
          <button className="ml-3 font-semibold underline" onClick={() => { autoStarted.current = false; void autoStart(); }}>
            重试
          </button>
        </div>)}
      ...
      <div className="min-h-0 flex-1 overflow-hidden flex">
        ...
        <div className="flex h-full flex-col overflow-hidden border-r border-gray-100 bg-gray-50/50" style={{ width: topology !== null ? (selectedAgentId ? '35%' : '50%') : '100%', transition: 'width 0.2s' }}>
          ...
          {/* Launching skeleton */}
          {isLaunching && allDisplayMessages.length === 0 && (<div className="flex flex-1 flex-col items-center justify-center gap-2 px-8 py-10">
              <div className="flex items-center gap-2">
          ...
          {/* Message thread */}
          {(!isLaunching || allDisplayMessages.length > 0) && (<div ref={chatScrollRef} className="flex-1 space-y-4 overflow-y-auto px-5 py-4">
              {allDisplayMessages.length === 0 ? (<div className="flex h-20 items-center justify-center rounded-xl border border-dashed border-gray-200 bg-white/60 text-xs text-gray-400">
                  等待 Supervisor 开始协调…
                </div>) : (allDisplayMessages.map((msg) => {
                // Coordination group — collapsible
                if (msg.role === "coordination-group") {
                    const grp = msg;
                    const expanded = expandedGroups.has(grp.id);
                    return (<div key={grp.id} className="pl-4">
                        ...
                        {expanded && (<div className="mt-1.5 space-y-1 border-l border-gray-100 pl-4">
                            {grp.items.map((item) => {
                ...
                if (m.isCoordination) {
                    ...
                    return (<div key={m.id} className="flex items-center gap-1.5 pl-6">
                        <ChevronRight className={`h-3 w-3 flex-shrink-0 ${COORDINATION_STYLE[type] ?? "text-gray-400"}`}/>
                ...
                // Chat bubble
                return (<div key={m.id} className={`flex items-start gap-2 ${m.role === "user" ? "justify-end" : "justify-start"}`}>
                      {m.role === "supervisor" && (<div className="mt-2.5 h-2 w-2 flex-shrink-0 rounded-full bg-blue-500"/>)}
                      <div className={`max-w-[80%] rounded-2xl px-4 py-3 text-sm ${m.role === "user"
                        ...
                        {m.isHitl && m.onBehalfOf && (<div className="mb-1.5 text-[11px] font-semibold text-yellow-600">
                            代 {resolveAgentName(m.onBehalfOf)} 询问
                          </div>)}
                        {m.role === "user" ? (<div className="whitespace-pre-wrap break-words">{m.text}</div>) : (() => {
                        const parsedQuestion = parseAskUserQuestion(m.text);
                        const displayContent = parsedQuestion ? removeYamlBlock(m.text) : m.text;
                        return (<>
                              {displayContent && (<div className="prose prose-sm max-w-none prose-p:leading-relaxed">
                                  <MarkdownContent content={displayContent}/>
                                </div>)}
                              {parsedQuestion && (<AskUserQuestionComponent parsedQuestion={parsedQuestion} onAnswer={(labels) => { void submitReply(labels.join(", "), m.workerId); }} disabled={isLaunching}/>)}
                            </>);
                    })()}
              ...
              {/* Thinking dots */}
              {isRunning && !isAwaitingHitl && (<div className="flex items-start gap-2 pl-4">
          ...
          {/* 多 HITL 并发卡片 */}
          {pendingHitlRequests.map((req) => (<HitlCard key={req.eventId} req={req} resolveAgentName={resolveAgentName} onReply={(answer) => {
                void submitReply(answer, req.workerId);
            }} disabled={isLaunching} uploading={uploading} onUpload={sessionId ? handleUpload : undefined}/>))}

          {/* Greeting / completed input area — hidden when HITL card is present */}
          {(isGreeting || isRunning || phase === "completed") && !isAwaitingHitl && (<ChatInputBar onSubmit={(msg) => { void submitReply(msg); }} disabled={isLaunching || pendingUserMessage !== null} placeholder={isGreeting ? "输入协作目标，按 Enter 发送…"
                : phase === "completed" ? "继续提问或发起新任务…"
                    : "回复 Supervisor…"} onUpload={sessionId ? handleUpload : undefined} uploadedFiles={uploadedFiles} onRemoveFile={(idx) => {
                if (idx === -1)
                    setUploadError(null);
                else
                    setUploadedFiles((prev) => prev.filter((_, i) => i !== idx));
            }} uploadError={uploadError} uploading={uploading} lightBg className="border-t border-gray-100 bg-white"/>)}
        </div>

        {/* Worker detail drawer */}
        {selectedAgentId !== null && (<div className="flex h-full flex-col overflow-hidden border-l border-gray-100 bg-white" style={{ width: '30%', minWidth: 280, transition: 'width 0.2s' }}>
            <WorkerDetailDrawer agentId={selectedAgentId} agentName={resolveAgentName(selectedAgentId)} activity={activities[selectedAgentId]} events={events} onClose={() => setSelectedAgentId(null)}/>
          </div>)}

        {/* Right: Topology panel */}
        {topology !== null && (<div className="flex h-full flex-col overflow-hidden bg-white" style={{ width: selectedAgentId ? '35%' : '50%', transition: 'width 0.2s' }}>
            <div className="flex flex-shrink-0 items-center gap-2 border-b border-gray-100 px-4 py-2.5">
              <Network className="h-3.5 w-3.5 text-gray-400"/>
              <span className="text-xs font-medium text-gray-600">协作拓扑</span>
            </div>
            <div className="flex-1 overflow-hidden">
              <TopologyGraph topology={topology} activities={activities} recentlyActiveAgents={recentlyActiveAgents} onNodeClick={(id) => {
