...
import { ingestPatternToArchival, ingestReflectionToArchival, migratePatternsToArchival } from '../archival/pattern-ingest';

export class EnhancedPatternProvider implements CognitiveProvider {
   ...
   * Agent 启动时调用，执行一次性迁移。
   */
  async initialize(): Promise<{ patternsMigrated: number; reflectionsMigrated: number }> {
    if (this.migrated) return { patternsMigrated: 0, reflectionsMigrated: 0 };
    this.migrated = true;
    return migratePatternsToArchival(this.archival, this.agentDir);
  }
  ...
  async sync_turn(data: TurnCognitiveData): Promise<void> {
    ...
    await ingestPatternToArchival(this.archival, {
      toolChain,
      successRate,
  ...
  async sync_reflection(data: {
    ...
    await ingestReflectionToArchival(this.archival, data);
  }
