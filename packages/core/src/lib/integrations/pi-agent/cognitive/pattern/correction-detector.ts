...
interface Rule {
  patterns: RegExp[];
  strength: CorrectionSignal['strength'];
}
...
export function detectCorrections(userMessage: string): CorrectionSignal[] {
...
/** 从信号列表中取最高强度 */
export function maxStrength(signals: CorrectionSignal[]): CorrectionSignal['strength'] | null {
  if (signals.some(s => s.strength === 'strong')) return 'strong';
  if (signals.some(s => s.strength === 'medium')) return 'medium';
  if (signals.some(s => s.strength === 'weak')) return 'weak';
  return null;
}
