...
async function handleUpdateStatus(instruction: string, tools: SkillContext['tools']): Promise<SkillResult> {
  ...
  const task = await tools.updateEntity!(taskId, { status }) as { id: string; properties: Record<string, unknown> } | null;
...
async function handleAssignTask(instruction: string, tools: SkillContext['tools']): Promise<SkillResult> {
  ...
  const personName = personMatch?.[1]?.trim() ?? null;
  ...
  const task = await tools.updateEntity!(taskId, { assignee: personName }) as { id: string; properties: Record<string, unknown> } | null;
...
async function handleTaskStats(tools: SkillContext['tools']): Promise<SkillResult> {
  const allTasks = await tools.queryEntities!('Task', {}) as Array<{ id: string; properties: Record<string, unknown> }>;
